...
HRESULT CDX9VideoProcessor::InitInternal(bool* pChangeDevice/* = nullptr*/)
{
	...
	if (S_OK == m_pD3DEx->GetDeviceCaps(m_nCurrentAdapter, D3DDEVTYPE_HAL, &DevCaps)) {
		CStdStringW dbgstr = L"DeviceCaps:";
...
HRESULT CDX9VideoProcessor::GetVPInfo(CStdStringW& str)
{
	str = L"DirectX 9";
	...
	if (m_DXVA2VP.IsReady()) {
		...
		str.AppendFormat(L"DXVA2 {}", DXVA2VPDeviceToString(DXVA2VPGuid));
...
void CDX9VideoProcessor::UpdateStatsStatic()
{
	if (m_srcParams.cformat) {
		m_strStatsHeader.Format(L"MPC VR {}, Direct3D 9Ex", _CRT_WIDE(VERSION_STR));

		UpdateStatsInputFmt();

		m_strStatsVProc.assign(L"\nVideoProcessor: ");
		if (m_DXVA2VP.IsReady()) {
			m_strStatsVProc.AppendFormat(L"DXVA2 VP, output to {}", D3DFormatToString(m_DXVA2OutputFmt));
		} else {
...
void CDX9VideoProcessor::UpdateStatsPostProc()
{
	if (m_strCorrection || m_pPostScaleShaders.size() || m_bFinalPass) {
		...
		if (m_strCorrection) {
			m_strStatsPostProc.AppendFormat(L" {},", m_strCorrection);
		}
		if (m_pPostScaleShaders.size()) {
			m_strStatsPostProc.AppendFormat(L" shaders[{}],", m_pPostScaleShaders.size());
		}
		if (m_bFinalPass) {
...
HRESULT CDX9VideoProcessor::DrawStats(IDirect3DSurface9* pRenderTarget)
{
	...
	if (m_strCorrection || m_pPostScaleShaders.size() || m_bDitherUsed) {
		str.append(L"\nPostProcessing:");
		if (m_strCorrection) {
			str.AppendFormat(L" {},", m_strCorrection);
		}
		if (m_pPostScaleShaders.size()) {
			str.AppendFormat(L" shaders[{}],", m_pPostScaleShaders.size());
		}
		if (m_bDitherUsed) {
			str.append(L" dither");
		}
...
#if SYNC_OFFSET_EX
	{
		...
		str.AppendFormat(L", range[{:+3.0f};{:+3.0f}], max change{:+3.0f}/{:+3.0f}",
			so_min / 10000.0f,
			so_max / 10000.0f,
			sod_min / 10000.0f,
			sod_max / 10000.0f);
