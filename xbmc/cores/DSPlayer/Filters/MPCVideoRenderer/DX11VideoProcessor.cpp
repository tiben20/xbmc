...
HRESULT CDX11VideoProcessor::AddPreScaleShader(const CStdStringW& name, const CStdString& srcCode)
{
#ifdef _DEBUG
	if (!m_pDevice) {
		return E_ABORT;
...
HRESULT CDX11VideoProcessor::AddPostScaleShader(const CStdStringW& name, const CStdString& srcCode)
{
	if (!m_pDevice) {
		return E_ABORT;
	}
